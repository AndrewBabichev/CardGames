...
class DatabaseHandler():
    ...
    def add_user(user_name, room_name):
        ...
        len_users = len(User.objects.filter(room_name=room_name, user_name=user_name))
        if len_users:
            user_name = user_name + "_" + str(len_users)
...
class CardGetter():
    ...
    def reset(self):
        self.cards = [(i, j) for i in range(0, 2)
                            for j in range(4, 13)]
...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def users_take(self):
        ...
        each_should_have = min(num_cards // n_users , 6)
        ...
        for user_name in user_names:
            ...
            if user_should_take == 0 or self.card_getter.size() == 0:
                continue
    ...
    async def receive(self, text_data):
        ...
        if type == 'look_up':
            ...
            user = await DatabaseHandler.add_user(self.user_name, self.room_name)
            ...
            if self.connected == self.num_players:
                await DatabaseHandler.reset_scores(self.room_name)
